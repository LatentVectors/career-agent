...
from .models import CandidateResponse, Experience
...
def _parse_response_section(content: str) -> dict[str, Any]:
    ...
    if not lines or not lines[0].startswith("# response_id:"):
        raise ValueError("Response section must start with '# response_id:'")
    ...
    if not id_match or not user_id_match:
        raise ValueError("Header must contain both response_id and user_id")
    ...
    return {"id": response_id, "user_id": user_id, "prompt": prompt, "response": response_content}
...
def _load_experiences(input_dir: Path, delete: bool = False) -> None:
    ...
    for filepath in input_dir.glob("experience_*.md"):
        try:
